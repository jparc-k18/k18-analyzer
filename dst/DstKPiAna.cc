...
namespace
{
  ...
  ClassName()
  {
    static TString s_name("DstKPiAna");
    return s_name;
  }
...
namespace dst
{
  ...
  const Double_t x_off = -0.3028; // created by 000430(260202)
  const Double_t y_off = -1.8383; // created by 000430(260202)
  const Double_t u_off = -0.0016; // created by 000430(260202)
  const Double_t v_off =  0.0006; // created by 000430(260202)
  const ThreeVector tgtSize(70.0, 50.0, 300.0);
  const ThreeVector tgtCenterOfs(-6.5, -14.0, 0.); // Old Li
  // const ThreeVector tgtCenterOfs(-1.0, -1.0, 0.); // New Li
}
...
struct Event
{
  ...
  Double_t MissMassSigmap[MaxHits];
  Double_t MissMassSigmapCorr[MaxHits];
  Double_t MissMassSigmapCorrDE[MaxHits];
  Double_t MissMassPi0[MaxHits];
  Double_t MissMassPi0Corr[MaxHits];
  Double_t MissMassPi0CorrDE[MaxHits];
  Double_t MissMassNuMuBar[MaxHits];
  Double_t MissMassNuMuBarCorr[MaxHits];
  Double_t MissMassNuMuBarCorrDE[MaxHits];
  Double_t BE[MaxHits];
  Double_t BECorrDE[MaxHits];
...
dst::InitializeEvent()
{
  ...
  for(Int_t it=0; it<MaxHits; ++it){
    ...
    event.MissMassSigmap[it]  = qnan;
    event.MissMassSigmapCorr[it]  = qnan;
    event.MissMassSigmapCorrDE[it]  = qnan;
    event.MissMassPi0[it]  = qnan;
    event.MissMassPi0Corr[it]  = qnan;
    event.MissMassPi0CorrDE[it]  = qnan;
    event.MissMassNuMuBar[it]  = qnan;
    event.MissMassNuMuBarCorr[it]  = qnan;
    event.MissMassNuMuBarCorrDE[it]  = qnan;
    event.BE[it]  = qnan;
    event.BECorrDE[it]  = qnan;
    event.Kflag[it] = 0;
...
dst::DstRead(Int_t ievent)
{
  ...
  static const auto MuonMass    = pdg::MuonMass();
  static const auto NeutronMass  = pdg::NeutronMass();
  ...
  static const Double_t Carbon11Mass = 11.*TGeoUnit::amu_c2 - 6.*ElectronMass;
  static const Double_t Li7Mass = 7.*TGeoUnit::amu_c2 - 3.*ElectronMass;
  static const Double_t Li6Mass = 6.*TGeoUnit::amu_c2 - 3.*ElectronMass;
  ...
  for(Int_t itS2s=0; itS2s<ntS2s; ++itS2s){
    ...
    HF1(4211, pCorr);
    HF1(4212, path);
    PiXCont.push_back(PosCorr);
    PiPCont.push_back(MomCorr);
  }
  ...
  for(Int_t ipi=0; ipi<nPi; ++ipi){
    ...
    for(Int_t ikm=0; ikm<nKm; ++ikm){
      ...
      ThreeVector vert = Kinematics::VertexPoint(xkm, xpi, pkm, ppi);
      // std::cout << "vertex : " << vert << " " << vert.Mag() << std::endl;
      Double_t closedist = Kinematics::CloseDist(xkm, xpi, pkm, ppi);

      Double_t cost = pkm*ppi/(pkm.Mag()*ppi.Mag());
      Double_t theta = TMath::ACos(cost)*TMath::RadToDeg();
      Double_t pk0   = ppi.Mag();
      Double_t pCorr = pk0;

      ThreeVector ppiCorr(pCorr*ppi.x()/ppi.Mag(),
                          pCorr*ppi.y()/ppi.Mag(),
                          pCorr*ppi.z()/ppi.Mag());

      // to be updated
      ThreeVector pkmCorrDE = pkm;
      ThreeVector ppiCorrDE = ppiCorr;
      // ThreeVector pkmCorrDE = Kinematics::CorrElossIn(pkm, xkm, vert, KaonMass);
      // ThreeVector ppiCorrDE = Kinematics::CorrElossOut(ppiCorr, xpi, vert, PionMass);
      ...
      LorentzVector LvPiCorr(ppiCorr, std::sqrt(PionMass*PionMass+ppiCorr.Mag2()));
      LorentzVector LvPiCorrDE(ppiCorrDE, std::sqrt(PionMass*PionMass+ppiCorrDE.Mag2()));
      LorentzVector LvMu(ppi, std::sqrt(MuonMass*MuonMass+ppi.Mag2()));
      LorentzVector LvMuCorr(ppiCorr, std::sqrt(MuonMass*MuonMass+ppiCorr.Mag2()));
      LorentzVector LvMuCorrDE(ppiCorrDE, std::sqrt(MuonMass*MuonMass+ppiCorrDE.Mag2()));

      //LorentzVector LvC(0., 0., 0., NeutronMass);
      LorentzVector LvC(0., 0., 0., Li7Mass);
      LorentzVector LvH(0., 0., 0., ProtonMass);
      LorentzVector LvEmp(0., 0., 0., 0.);
      LorentzVector LvCore(0., 0., 0., 0.);

      LorentzVector LvRc       = LvKm+LvC-LvPi;
      LorentzVector LvRcCorr   = LvKm+LvC-LvPiCorr;
      LorentzVector LvRcCorrDE = LvKmCorrDE+LvC-LvPiCorrDE;
      LorentzVector LvRcSigmap       = LvKm+LvH-LvPi;
      LorentzVector LvRcSigmapCorr   = LvKm+LvH-LvPiCorr;
      LorentzVector LvRcSigmapCorrDE = LvKmCorrDE+LvH-LvPiCorrDE;
      LorentzVector LvRcPi0       = LvKm+LvEmp-LvPi;
      LorentzVector LvRcPi0Corr   = LvKm+LvEmp-LvPiCorr;
      LorentzVector LvRcPi0CorrDE = LvKmCorrDE+LvEmp-LvPiCorrDE;
      LorentzVector LvRcNuMuBar       = LvKm+LvEmp-LvMu;
      LorentzVector LvRcNuMuBarCorr   = LvKm+LvEmp-LvMuCorr;
      LorentzVector LvRcNuMuBarCorrDE = LvKmCorrDE+LvEmp-LvMuCorrDE;
      Double_t MissMass       = LvRc.Mag();
      Double_t MissMassCorr   = LvRcCorr.Mag();
      Double_t MissMassCorrDE = LvRcCorrDE.Mag();
      Double_t MissMassSigmap       = LvRcSigmap.Mag();
      Double_t MissMassSigmapCorr   = LvRcSigmapCorr.Mag();
      Double_t MissMassSigmapCorrDE = LvRcSigmapCorrDE.Mag();
      Double_t MissMassPi0       = LvRcPi0.Mag();
      Double_t MissMassPi0Corr   = LvRcPi0Corr.Mag();
      Double_t MissMassPi0CorrDE = LvRcPi0CorrDE.Mag();
      Double_t MissMassNuMuBar       = LvRcNuMuBar.Mag();
      Double_t MissMassNuMuBarCorr   = LvRcNuMuBarCorr.Mag();
      Double_t MissMassNuMuBarCorrDE = LvRcNuMuBarCorrDE.Mag();

      Double_t BE = MissMassCorr-(Li6Mass+LambdaMass);
      Double_t BECorrDE = MissMassCorrDE-(Li6Mass+LambdaMass);
	...
	= 0.5*std::sqrt((TotalEnergyCM*TotalEnergyCM
                         -(PionMass+LambdaMass)*(PionMass+LambdaMass))
			*(TotalEnergyCM*TotalEnergyCM
                          -(PionMass-LambdaMass)*(PionMass-LambdaMass)))/TotalEnergyCM;

      Double_t us = ppi.x()/ppi.z(), vs = ppi.y()/ppi.z();
      ...
      Double_t gbep=gamma*bt*std::sqrt(TotalMomCM*TotalMomCM+PionMass*PionMass)/TotalMomCM;
	...
	costCM=-1.;
      ...
      if(nkpi < MaxHits){
	event.vtx[nkpi]=vert.x();
	event.vty[nkpi]=vert.y();
	event.vtz[nkpi]=vert.z();
	event.closeDist[nkpi] = closedist;
	event.inside[nkpi]  = inside;
	event.theta[nkpi]     = theta;
	event.thetaCM[nkpi]   = TMath::ACos(costCM)*TMath::RadToDeg();
	event.costCM[nkpi]    = costCM;

	event.MissMass[nkpi]       = MissMass;
	event.MissMassCorr[nkpi]   = MissMassCorr;
	event.MissMassCorrDE[nkpi] = MissMassCorrDE;
	event.MissMassSigmap[nkpi]       = MissMassSigmap;
	event.MissMassSigmapCorr[nkpi]   = MissMassSigmapCorr;
	event.MissMassSigmapCorrDE[nkpi] = MissMassSigmapCorrDE;
	event.MissMassPi0[nkpi]       = MissMassPi0;
	event.MissMassPi0Corr[nkpi]   = MissMassPi0Corr;
	event.MissMassPi0CorrDE[nkpi]   = MissMassPi0CorrDE;
	event.MissMassNuMuBar[nkpi]       = MissMassNuMuBar;
	event.MissMassNuMuBarCorr[nkpi]   = MissMassNuMuBarCorr;
	event.MissMassNuMuBarCorrDE[nkpi]   = MissMassNuMuBarCorrDE;
	event.BE[nkpi] = BE;
	event.BECorrDE[nkpi] = BECorrDE;

	event.xpi[nkpi] = xpi.x();
	event.ypi[nkpi] = xpi.y();
	event.upi[nkpi] = us;
	event.vpi[nkpi] = vs;
	event.xkm[nkpi] = xkm.x();
	event.ykm[nkpi] = xkm.y();
	event.ukm[nkpi] = ub;
	event.vkm[nkpi] = vb;
	event.pOrg[nkpi] = pk0;
	event.pCalc[nkpi] = pCalc;
	event.pCorr[nkpi] = pCorr;
	//	event.pCorrDE[nkpi] = ppiCorrDE.Mag();
	event.pCorrDE[nkpi] = 0;
	nkpi++;
      }else{
	std::cout << FUNC_NAME << " nkpi("
                  << nkpi << ") exceeding MaxHits: " << MaxHits << std::endl;
      }

      // Good event histograms
      // GoodTrack
      if(nPi == 1 && nKm == 1
	 && event.chisqrK18[ikm] < 20.
         && event.chisqrS2s[ipi] < 20.){
        HF1(6001, event.pK18[ikm]);
        HF1(6002, event.pS2s[ipi]);
        HF1(6003, event.qS2s[ipi]*event.m2[ipi]);
        HF2(6004, event.qS2s[ipi]*event.m2[ipi], event.pS2s[ipi]);
        HF1(6011, closedist);
        HF1(6012, vert.x());
        HF1(6013, vert.y());
        HF1(6014, vert.z());
	...
	HF1(6025, theta);
	HF2(6026, event.pS2s[ipi], theta);
	HF1(6027, MissMassPi0Corr);
	HF1(6028, MissMassNuMuBarCorr);
	HF2(6029, vert.z(), MissMassCorr);
	HF1(6045, MissMassCorrDE);
	HF1(6046, BECorrDE);
	HF1(6057, MissMassPi0CorrDE);
	HF1(6058, MissMassNuMuBarCorrDE);
		
	// GoodVertex
        if(inside == 1 && theta>2.){
          HF1(6101, event.pK18[ikm]);
          HF1(6102, event.pS2s[ipi]);
          HF1(6103, event.qS2s[ipi]*event.m2[ipi]);
          HF2(6104, event.qS2s[ipi]*event.m2[ipi], event.pS2s[ipi]);
          HF1(6111, closedist);
          HF1(6112, vert.x());
          HF1(6113, vert.y());
          HF1(6114, vert.z());
	  ...
	  HF1(6125, theta);
	  HF2(6126, event.pS2s[ipi], theta);
	  HF1(6127, MissMassPi0Corr);
	  HF1(6128, MissMassNuMuBarCorr);
	  HF2(6129, vert.z(), MissMassCorr);
	  HF1(6145, MissMassCorrDE);
	  HF1(6146, BECorrDE);
	  HF1(6157, MissMassPi0CorrDE);
	  HF1(6158, MissMassNuMuBarCorrDE);
	  
	  // M2forPion
	  if(event.m2[ipi] > -0.08
	     && event.m2[ipi] < 0.08
	     // && event.trigflag[20]>0
	     ){
	    HF1(6201, event.pK18[ikm]);
	    HF1(6202, event.pS2s[ipi]);
	    HF1(6203, event.qS2s[ipi]*event.m2[ipi]);
	    HF2(6204, event.qS2s[ipi]*event.m2[ipi], event.pS2s[ipi]);
	    HF1(6211, closedist);
	    HF1(6212, vert.x());
	    HF1(6213, vert.y());
	    HF1(6214, vert.z());
	    ...
	    HF1(6225, theta);
	    HF2(6226, event.pS2s[ipi], theta);
	    HF1(6227, MissMassPi0Corr);
	    HF1(6228, MissMassNuMuBarCorr);
	    HF2(6229, vert.z(), MissMassCorr);
	    HF1(6245, MissMassCorrDE);
	    HF1(6246, BECorrDE);
	    HF1(6257, MissMassPi0CorrDE);
	    HF1(6258, MissMassNuMuBarCorrDE);

	    // GoodKPi
	    if(theta < 6.0){
	      HF1(6301, event.pK18[ikm]);
	      HF1(6302, event.pS2s[ipi]);
	      HF1(6303, event.qS2s[ipi]*event.m2[ipi]);
	      HF2(6304, event.qS2s[ipi]*event.m2[ipi], event.pS2s[ipi]);
	      HF1(6311, closedist);
	      HF1(6312, vert.x());
	      HF1(6313, vert.y());
	      HF1(6314, vert.z());
	      ...
	      HF1(6325, theta);
	      HF2(6326, event.pS2s[ipi], theta);
	      HF1(6327, MissMassPi0Corr);
	      HF1(6328, MissMassNuMuBarCorr);
	      HF2(6329, vert.z(), MissMassCorr);
	      HF1(6345, MissMassCorrDE);
	      HF1(6346, BECorrDE);
	      HF1(6357, MissMassPi0CorrDE);
	      HF1(6358, MissMassNuMuBarCorrDE);
	    }
	  }// M2forPion
        }// GoodVertex
      }// GoodTrack
...
{
  const Int_t NBinP = 350;
  const Double_t MinP = 0.5;
  const Double_t MaxP = 1.2;
  const Int_t NBinM2 = 800;
  const Double_t MinM2 = -1.4;
  const Double_t MaxM2 =  2.0;
  const Int_t NBinMM = 400;
  const Double_t MinMM = 11.2;
  const Double_t MaxMM = 11.6;
  const Int_t NBinBE = 800;
  const Double_t MinBE = -0.2;
  const Double_t MaxBE =  0.2;
  ...
  HB1(152, "ClusterSize BH2 [kpi]", 7, 0., 7.);
  HB1(153, "HitPat BH2 [kpi]", 10, 0., 10.);
  HB1(154, "MeanTime BH2 [kpi]", 200, -10., 10.);
  HB1(155, "Delta-E BH2 [kpi]", 200, -0.5, 4.5);
  ...
  HB1(252, "ClusterSize BH1 [kpi]",11, 0., 11.);
  HB1(253, "HitPat BH1 [kpi]", 11, 0., 11.);
  HB1(254, "MeanTime BH1 [kpi]", 200, -10., 10.);
  HB1(255, "Delta-E BH1 [kpi]", 200, -0.5, 4.5);
  HB1(256, "Beam ToF [kpi]", 200, -10., 10.);
  ...
  HB1(352, "ClusterSize Tof [kpi]",32, 0., 32.);
  HB1(353, "HitPat Tof [kpi]", 32, 0., 32.);
  HB1(354, "TimeOfFlight Tof [kpi]", 500, -50., 100.);
  HB1(355, "Delta-E Tof [kpi]", 200, -0.5, 4.5);
  ...
  { 
    std::vector<TString> labels = {
      "[GoodTrack]", "[GoodVertex]", "[M2forPion]", "[GoodKPi]"
    };
    for(Int_t i=0, n=labels.size(); i<n; ++i){
      Int_t hofs = 6000 + i*100;
      HB1(hofs+ 1, "P K18 "+labels[i], NBinP, MinP, MaxP);
      HB1(hofs+ 2, "P S2s "+labels[i], NBinP, MinP, MaxP);
      HB1(hofs+ 3, "Charge*MassSquared "+labels[i], 800, -1., 2.);
      HB2(hofs+ 4, "P S2s%Charge*MassSquared "+labels[i],
	  800, -1., 2., NBinP, MinP, MaxP);
      ...
      HB1(hofs+15, "MissingMass "+labels[i], NBinMM, MinMM, MaxMM);
      HB1(hofs+16, "BE "+labels[i], NBinBE, MinBE, MaxBE);
      HB2(hofs+17, "#DeltaP%U "+labels[i], 200, -0.5, 0.5, 200, -1., 1.);
      HB2(hofs+18, "MissingMass%U "+labels[i], 200, -0.5, 0.5, NBinMM, MinMM, MaxMM);
      HB2(hofs+19, "#DeltaP%V "+labels[i], 200, -0.5, 0.5, 200, -1., 1.);
      HB2(hofs+20, "MissingMass%V "+labels[i], 200, -0.5, 0.5, NBinMM, MinMM, MaxMM);
      ...
      HB1(hofs+25, "theta "+labels[i], 200, 0., 20.);
      HB2(hofs+26, "P S2s % theta "+labels[i], NBinP, MinP, MaxP, 200, 0., 20.);
      HB1(hofs+27, "MissingMassPi0 "+labels[i], 600, -0.30, 0.30);
      HB1(hofs+28, "MissingMassNuMu "+labels[i], 600, -0.30, 0.30);
      HB2(hofs+29, "MissingMass%Vtz "+labels[i], 400, -1000., 1000, NBinMM, MinMM, MaxMM);
      HB1(hofs+45, "MissingMassCorrDE "+labels[i], NBinMM, MinMM, MaxMM);
      HB1(hofs+46, "BECorrDE "+labels[i], NBinBE, MinBE, MaxBE);
      HB1(hofs+57, "MissingMassPi0CorrDE "+labels[i], 600, -0.30, 0.30);
      HB1(hofs+58, "MissingMassNuMuCorrDE "+labels[i], 600, -0.30, 0.30);
    }
  }

  ////////////////////////////////////////////
  //Tree
  HBTree("kpi","tree of KPiAna");
  ...
  tree->Branch("nKPi",           &event.nKPi,            "nKPi/I");
  tree->Branch("vtx",            event.vtx,            "vtx[nKPi]/D");
  tree->Branch("vty",            event.vty,            "vty[nKPi]/D");
  tree->Branch("vtz",            event.vtz,            "vtz[nKPi]/D");
  tree->Branch("closeDist",      event.closeDist,      "closeDist[nKPi]/D");
  tree->Branch("inside",         event.inside,         "inside[nKPi]/I");
  tree->Branch("theta",          event.theta,          "theta[nKPi]/D");
  tree->Branch("MissMass",       event.MissMass,       "MissMass[nKPi]/D");
  tree->Branch("MissMassCorr",   event.MissMassCorr,   "MissMassCorr[nKPi]/D");
  tree->Branch("MissMassCorrDE", event.MissMassCorrDE, "MissMassCorrDE[nKPi]/D");
  tree->Branch("MissMassSigmap",       event.MissMassSigmap,       "MissMassSigmap[nKPi]/D");
  tree->Branch("MissMassSigmapCorr",   event.MissMassSigmapCorr,   "MissMassSigmapCorr[nKPi]/D");
  tree->Branch("MissMassSigmapCorrDE", event.MissMassSigmapCorrDE, "MissMassSigmapCorrDE[nKPi]/D");
  tree->Branch("MissMassPi0",       event.MissMassPi0,       "MissMassPi0[nKPi]/D");
  tree->Branch("MissMassPi0Corr",   event.MissMassPi0Corr,   "MissMassPi0Corr[nKPi]/D");
  tree->Branch("MissMassPi0CorrDE",   event.MissMassPi0CorrDE,   "MissMassPi0CorrDE[nKPi]/D");
  tree->Branch("MissMassNuMuBar",       event.MissMassNuMuBar,       "MissMassNuMuBar[nKPi]/D");
  tree->Branch("MissMassNuMuBarCorr",   event.MissMassNuMuBarCorr,   "MissMassNuMuBarCorr[nKPi]/D");
  tree->Branch("MissMassNuMuBarCorrDE",   event.MissMassNuMuBarCorrDE,   "MissMassNuMuBarCorrDE[nKPi]/D");
  tree->Branch("BE", event.BE, "BE[nKPi]/D");
  tree->Branch("BECorrDE", event.BECorrDE, "BECorrDE[nKPi]/D");
  tree->Branch("thetaCM", event.thetaCM,  "thetaCM[nKPi]/D");
  tree->Branch("costCM",  event.costCM,   "costCM[nKPi]/D");
  tree->Branch("Kflag" ,  event.Kflag,    "Kflag[nKPi]/I");

  tree->Branch("xkm",        event.xkm,      "xkm[nKPi]/D");
  tree->Branch("ykm",        event.ykm,      "ykm[nKPi]/D");
  tree->Branch("ukm",        event.ukm,      "ukm[nKPi]/D");
  tree->Branch("vkm",        event.vkm,      "vkm[nKPi]/D");
  tree->Branch("xpi",        event.xpi,      "xpi[nKPi]/D");
  tree->Branch("ypi",        event.ypi,      "ypi[nKPi]/D");
  tree->Branch("upi",        event.upi,      "upi[nKPi]/D");
  tree->Branch("vpi",        event.vpi,      "vpi[nKPi]/D");
  tree->Branch("pOrg",       event.pOrg,     "pOrg[nKPi]/D");
  tree->Branch("pCalc",      event.pCalc,    "pCalc[nKPi]/D");
  tree->Branch("pCorr",      event.pCorr,    "pCorr[nKPi]/D");
  tree->Branch("pCorrDE",    event.pCorrDE,  "pCorrDE[nKPi]/D");
  ...
  //TTreeCont[kK18Tracking]->SetBranchStatus("thetaK18",    1);
  TTreeCont[kK18Tracking]->SetBranchStatus("theta",       1); //k18 theta is implemented as "theta" in UserK18Tracking.cc
  ...
  //TTreeCont[kK18Tracking]->SetBranchAddress("thetaK18", src.thetaK18);
  TTreeCont[kK18Tracking]->SetBranchAddress("theta", src.thetaK18);//k18 theta is implemented as "theta" in UserK18Tracking.cc
