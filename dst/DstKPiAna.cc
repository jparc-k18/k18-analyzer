...
dst::DstRead(Int_t ievent)
{
  ...
  for(Int_t ikp=0; ikp<nKp; ++ikp){
    ...
    for(Int_t ikm=0; ikm<nKm; ++ikm){
      ...
      LorentzVector LvKpCorr(pkpCorr, std::sqrt(PionMass*PionMass+pkpCorr.Mag2()));
      ...
      Double_t BE = MissMassCorr-SigmaNMass;
	...
	= 0.5*std::sqrt((TotalEnergyCM*TotalEnergyCM
                         -(PionMass+SigmaNMass)*(PionMass+SigmaNMass))
			*(TotalEnergyCM*TotalEnergyCM
                          -(PionMass-SigmaNMass)*(PionMass-SigmaNMass)))/TotalEnergyCM;
	...
	std::cout << FUNC_NAME << " nkk("
      ...
      if(event.chisqrK18[ikm] < 20.
        ...
        if(inside == 1){
          ...
          if(event.pKurama[ikp] < 1.6 && event.qKurama[ikp] > 0){
...
ConfMan::InitializeHistograms()
{
  ...
  for(Int_t i=0, n=labels.size(); i<n; ++i){
    ...
    HB1(hofs+15, "MissingMass "+labels[i], 200, 0.8, 1.8);
