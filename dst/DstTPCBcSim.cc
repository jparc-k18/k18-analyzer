...
namespace dst
{
	...
	{
		kProcess, kConfFile,
		kBcOut, kOutFile, nArgc
	};
	std::vector<TString> ArgName =
	{ "[Process]", "[ConfFile]", "[BcOut]", "[OutFile]" };
	std::vector<TString> TreeName = { "", "", "bcout" ,"" };
	std::vector<TFile*> TFileCont;
	std::vector<TTree*> TTreeCont;
	std::vector<TTreeReader*> TTreeReaderCont;
	TH2Poly* padHist;
	TH2Poly* InitializePadHist(){
		Double_t X[5]; Double_t Y[5];
		TH2Poly* padHist = new TH2Poly("PadHist","PadHist",-250,250,-250,250);
		for (int l = 0 ; l < 32; l++) {
...
//	gRandom->SetSeed(0);
	...
	CatchSignal::Set();
	std::cout<<"Initializing PadHistogram"<<std::endl;
	dst::padHist = dst::InitializePadHist();
	Int_t ievent = skip;
...
dst::DstRead(int ievent)
{
	...
	double shared_mem = share * page_size_kb/1024;
	std::cout << "Shared Memory - " << shared_mem << " MB\n";

	std::cout << "Private Memory - " << rss - shared_mem << " MB\n";
#endif
	...
	dst::padHist->Reset("");
	PadSimulator PadSim;
	PadSim.SetNumberOfPoints(1000);
	PadSim.Simulate(event.chisqrBcOut,
			event.x0BcOut,
			event.y0BcOut,
			event.u0BcOut,
			event.v0BcOut,
	...
	for(Int_t layer=0; layer<NumOfLayersTPC; ++layer){
		auto hc = PadSim.GetTPCHC(layer);
		for(const auto& hit : hc){
			if(!hit )continue;
//			std::cout<<"Layer "<<layer<<" Addr : "<<hit<<std::endl;
	...
	Int_t nclTpc = 0;
	for(Int_t layer=0; layer<NumOfLayersTPC; ++layer){
		auto hc = PadSim.GetTPCClCont(layer);
...
#if TrackSearch
	PadSim.TrackSearchTPC();
#endif

	Int_t ntTpc = PadSim.GetNTracksTPC();
	event.ntTpc = ntTpc;
	HF1(10, ntTpc);
	if(event.ntTpc == 0)
		return true;
	...
	for(Int_t it=0; it<ntTpc; ++it){
		TPCLocalTrack *tp = PadSim.GetTrackTPC(it);
		if(!tp) continue;
		Int_t nh = tp->GetNHit();
...
{
	const Int_t    NbinDe = 100;
	const Double_t MinDe  =    0.;
	const Double_t MaxDe  = 100.;
	const Int_t    NbinHitDe = 100;
	const Double_t MinHitDe  =   0.;
	const Double_t MaxHitDe  = 100.;
#if PositionCorrection
	...
	HBTree("tpc", "tree of DstTPCBcSim");
