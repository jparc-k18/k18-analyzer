...
HodoWave1Hit::HodoWave1Hit( HodoRawHit *rhit, int ref_sample )
  : HodoHit(rhit), m_raw(rhit), m_is_calculated(false), m_ref_sample(ref_sample)
{
...
HodoWave1Hit::Calculate( bool tdc_flag )
{
  ...
  // hit information
  m_multi_hit_l = m_raw->GetSizeTdcLeading()  > m_raw->GetSizeTdcTrailing()?  m_raw->GetSizeTdcLeading()  : m_raw->GetSizeTdcTrailing();
  //  m_multi_hit_t = m_raw->SizeTdcT1() > m_raw->SizeTdcT2()? m_raw->SizeTdcT1() : m_raw->SizeTdcT2();
  m_multi_hit_t = m_raw->GetSizeTdcTrailing() > m_raw->GetSizeTdcLeading()? m_raw->GetSizeTdcTrailing() : m_raw->GetSizeTdcLeading();
  int UorD=0;
  int nh_fadc = m_raw->GetSizeAdcHigh();
  ...
  for( int m=0; m<mhit; ++m ){
    ...
    double time = -999.;
    if(0 == UorD){
      tdc = m_raw->GetTdcUp(m);
    }else{
      tdc = m_raw->GetTdcDown(m);
    }
